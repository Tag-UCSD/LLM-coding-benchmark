"""
Process Llama (via Groq) coding results into CSV format
"""
...
def extract_codes_from_llama_responses_per_code(base_path, passages_include=range(9, 120)):
    """
    Extract codes from Llama per-code responses.

    Args:
        base_path: Path to per-code responses (e.g., 'results/raw/output_llama/per-code-with-justification_t=0_model=llama-3.1-70b')
...
def process_llama_per_code_condition(condition_path):
    """Process a single Llama per-code condition and save to CSV."""
    ...
    # Extract codes
    df = extract_codes_from_llama_responses_per_code(condition_path)
...
def process_all_llama_results():
    """Process all Llama coding results."""

    print("=" * 80)
    print("PROCESSING LLAMA RESULTS")
    print("=" * 80)

    conditions = [
        'results/raw/output_llama/per-code-with-justification_t=0_model=llama-3.3-70b',
        'results/raw/output_llama/per-code-without-justification_t=0_model=llama-3.3-70b',
    ]

    results = {}
    for condition in conditions:
        df = process_llama_per_code_condition(condition)
        if df is not None:
