...
Updated to include Gemini alongside GPT models (and Llama/Ollama)
Calculates intercoder reliability metrics comparing LLM coding to gold standard
"""
...
def gold_standard_vs_all_models_comparison():
    """Compare all model conditions (GPT-4, GPT-3.5, Gemini, and Llama) against gold standard."""
    ...
    dirs = {
        ...
        # GPT-3.5 conditions
        'GPT-3.5: Per-Code w/ Just': 'results/raw/output_original/per-code-with-justification_t=0_top_p=1_model=gpt-3.5',
        'GPT-3.5: Per-Code w/o Just': 'results/raw/output_original/per-code-without-justification_t=0_top_p=1_model=gpt-3.5',

        # Gemini conditions (only with-justification available)
        'Gemini: Per-Code w/ Just': 'results/raw/output_gemini/per-code-with-justification_t=0_top_p=1_model=gemini',
        'Gemini: Full w/ Just': 'results/raw/output_gemini/full-codebook_with-justification_t=0_top_p=1_model=gemini',

        # Llama (per-code with justification, via Groq/Ollama)
        'Llama: Per-Code w/ Just': 'results/raw/output_llama/per-code-with-justification_t=0_model=llama-3.3-70b',
    }
...
def main():
    ...
    print("=" * 80)
    print("LLM Qualitative Coding Analysis: GPT-4, GPT-3.5, Gemini, and Llama")
    print("=" * 80)

    # Create output directory
    os.makedirs('output', exist_ok=True)

    # Run gold standard vs all models comparison
    print("\nCalculating intercoder reliability metrics...")
    comparison_df = gold_standard_vs_all_models_comparison()

    print("\nResults:")
    print(comparison_df.to_string(index=False))

    # Save results
    output_file = 'results/outputs/intercoder_reliability_results_with_gemini.csv'
    comparison_df.to_csv(output_file, index=False)
    print(f"\nResults saved to: {output_file}")

    # Also generate individual condition reports for Gemini
    print("\nGenerating individual condition reports for Gemini...")

    gemini_conditions = {
        'gemini-per-code-with-justification': 'results/raw/output_gemini/per-code-with-justification_t=0_top_p=1_model=gemini',
        'gemini-full-codebook-with-justification': 'results/raw/output_gemini/full-codebook_with-justification_t=0_top_p=1_model=gemini',
    }

    for condition_name, dir_path in gemini_conditions.items():
        file_path = f'{dir_path}/processed_responses.csv'
    ...
    # Llama results (per-code with justification)
    print("\nGenerating individual condition reports for Llama...")
    llama_path = 'results/raw/output_llama/per-code-with-justification_t=0_model=llama-3.3-70b/processed_responses.csv'
    if os.path.exists(llama_path):
        pair = ['data/processed/gold_standard_coding.csv', llama_path]
        report_df = get_ir_report_df(pair, coded_columns, ids=row_ids, id_column='id')

        detail_file = 'results/outputs/llama-per-code-with-justification_detailed_report.csv'
        report_df.to_csv(detail_file, index=False)
        print(f"  - llama-per-code-with-justification: saved to {detail_file}")
    else:
        print("  - Llama results not found, skipping detailed report")
