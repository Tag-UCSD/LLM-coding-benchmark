"""
Qualitative Coding with Mistral-Small via HuggingFace API
...
def initialize_mistral(api_token):
...
def make_mistral_call(prompt, message, model="mistralai/Mistral-Small-Instruct-2409", temperature=0.0, max_retries=3):
    """Make a single Mistral API call."""
...
def make_and_save_mistral_calls(prompt, messages, path, model="mistralai/Mistral-Small-Instruct-2409", temperature=0):
    """Make Mistral API calls and save responses."""
    ...
    for msg in messages:
        ...
        try:
            ...
            reply = make_mistral_call(prompt, msg, model=model, temperature=temperature)
...
def run_per_code_mistral(api_token, model="mistralai/Mistral-Small-Instruct-2409", with_justification=True):
    """Run per-code coding with Mistral."""

    print("\n" + "=" * 80)
    print(f"MISTRAL PER-CODE CODING (with_justification={with_justification})")
    print("=" * 80)

    initialize_mistral(api_token)
    ...
    base_path = f'output_mistral/per-code-{just_str}_t=0_model=mistral-small'
    ...
    for code in codes_full:
        ...
        make_and_save_mistral_calls(prompt, messages, code_path, model=model, temperature=0)
    ...
    print(f"\n✓ Completed Mistral per-code coding")
    return base_path


if __name__ == "__main__":
    print("\nMistral Coding Workflow")
    print("Requires HuggingFace API token")
    print("\nUsage:")
    print("  from mistral_coding_workflow import run_per_code_mistral")
    print("  run_per_code_mistral('YOUR_HF_TOKEN')")
