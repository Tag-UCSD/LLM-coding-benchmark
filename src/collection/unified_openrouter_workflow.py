"""
Unified OpenRouter workflow for all models with proper passage alignment
Includes automatic rate limit handling and resumption
"""
...
MODELS = {
    ...
    'llama': 'meta-llama/llama-3.3-70b-instruct:free',  # Free tier!
...
def make_openrouter_call(prompt, message, model, temperature=0.0, max_retries=10):
    """
    Make a single OpenRouter API call with automatic rate limit handling.

    Features:
    - Exponential backoff for rate limits
    - Automatic retry on rate limit errors
    - Waits for rate limit reset when specified
    """
    client = OpenAI(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'rate' in error_msg.lower() or '429' in error_msg:
                ...
                import re
                time_match = re.search(r'(\d+)\s*second', error_msg, re.IGNORECASE)
                if time_match:
                    wait_time = int(time_match.group(1)) + 5  # Add buffer
...
def make_and_save_openrouter_calls(prompt, passages_df, path, model, temperature=0):
    """
    Make OpenRouter API calls and save responses.
    Uses passage IDs from gold standard for proper alignment.
    """
    ...
    for idx, row in passages_df.iterrows():
        ...
        if os.path.exists(f"{path}/message_{passage_id}.txt"):
            skipped += 1
            continue

        try:
            print(f"  Processing passage {passage_id} ({completed + 1}/{total - skipped})...")
            reply = make_openrouter_call(prompt, passage_text, model=model, temperature=temperature)

            all_messages_and_replies.append({"role": "user", "content": passage_text})
            all_messages_and_replies.append({"role": "assistant", "content": reply})

            with open(f"{path}/message_{passage_id}.txt", "w") as f:
                f.write(f"User: {passage_text}\n")
            ...
            print(f"  ✗ Error processing passage {passage_id}: {str(e)}")
...
def run_per_code_openrouter(api_key, model_name, model_id, with_justification=True):
    """Run per-code coding with OpenRouter using CORRECT passage IDs."""

    print("\n" + "=" * 80)
    print(f"{model_name.upper()} PER-CODE CODING (with_justification={with_justification})")
    print(f"Model: {model_id}")
    print("=" * 80)

    initialize_openrouter(api_key)

    # CRITICAL FIX: Load passages from GOLD STANDARD with correct IDs
    print("\nLoading passages from gold standard (correct IDs)...")
    gold_standard = pd.read_csv("data/processed/gold_standard_coding.csv")

    # Filter to test set (IDs 9-119)
    test_set_ids = range(9, 120)
    test_passages = gold_standard[gold_standard['id'].isin(test_set_ids)].copy()

    print(f"  ✓ Loaded {len(test_passages)} passages from test set")
    print(f"  ✓ ID range: {test_passages['id'].min()} to {test_passages['id'].max()}")
    print(f"  ✓ IDs are from gold standard (scattered across 0-232)")

    just_str = 'with-justification' if with_justification else 'without-justification'
    base_path = f'results/raw/output_{model_name}/per-code-{just_str}_t=0_model={model_name}'
    ...
    for code in codes_full:
        ...
        make_and_save_openrouter_calls(prompt, test_passages[['id', 'passage']], code_path, model=model_id, temperature=0)
    ...
    params = {
        ...
        'passage_source': 'gold_standard_coding.csv',
    ...
    print(f"\n✓ Completed {model_name} per-code coding")
    return base_path


if __name__ == "__main__":
    print("\nUnified OpenRouter Coding Workflow")
    print("Supports: Qwen, Llama, Gemini via OpenRouter")
    print("\nUsage:")
    print("  from unified_openrouter_workflow import run_per_code_openrouter, MODELS")
    print("  run_per_code_openrouter('YOUR_API_KEY', 'qwen', MODELS['qwen'])")
