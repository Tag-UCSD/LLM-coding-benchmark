"""
Qualitative Coding with Llama via Groq API
Parallel comparison to GPT-4 and Gemini
"""
...
# Reuse code definitions from Gemini workflow
codes_full = [
    {
        'title': 'Scholar',
...
def make_groq_call(prompt, message, model="llama-3.3-70b-versatile", temperature=0.0, max_retries=3):
...
def make_and_save_groq_calls(prompt, messages, path, model="llama-3.3-70b-versatile", temperature=0):
    """Make Groq API calls and save responses."""
    ...
    for msg in messages:
        ...
        try:
            ...
            reply = make_groq_call(prompt, msg, model=model, temperature=temperature)
...
def run_per_code_llama(api_key, model="llama-3.3-70b-versatile", with_justification=True):
    """Run per-code coding with Llama via Groq."""

    print("\n" + "=" * 80)
    print(f"LLAMA PER-CODE CODING (with_justification={with_justification})")
    print(f"Model: {model}")
    print("=" * 80)
    ...
    base_path = f'results/raw/output_llama/per-code-{just_str}_t=0_model=llama-3.3-70b'
    ...
    for code in codes_full:
        ...
        make_and_save_groq_calls(prompt, messages, code_path, model=model, temperature=0)
    ...
    print(f"\n✓ Completed Llama per-code coding")
    return base_path


def run_full_codebook_llama(api_key, model="llama-3.3-70b-versatile", with_justification=True):
    """Run full-codebook coding with Llama via Groq."""

    print("\n" + "=" * 80)
    print(f"LLAMA FULL-CODEBOOK CODING (with_justification={with_justification})")
    print(f"Model: {model}")
    print("=" * 80)
    ...
    just_str = 'with-justification' if with_justification else 'without-justification'
    base_path = f'results/raw/output_llama/full-codebook-{just_str}_t=0_model=llama-3.3-70b'

    print(f"\nProcessing full codebook approach...")

    if not os.path.exists(base_path):
        os.makedirs(base_path)

    prompt = generate_full_codebook_prompt(with_justification=with_justification)
    make_and_save_groq_calls(prompt, messages, base_path, model=model, temperature=0)
    ...
    print(f"\n✓ Completed Llama full-codebook coding")
    return base_path


if __name__ == "__main__":
    print("\nLlama (via Groq) Coding Workflow")
    print("Requires Groq API key (free at https://console.groq.com)")
    print("\nUsage:")
    print("  from groq_coding_workflow import run_per_code_llama, run_full_codebook_llama")
    print("  run_per_code_llama('YOUR_GROQ_API_KEY')")
    print("  run_full_codebook_llama('YOUR_GROQ_API_KEY')")
